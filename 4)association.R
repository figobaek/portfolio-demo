...
for(i in 1:length(min)){
  ...
  nouns <- str_replace_all(nouns, '인상[가-힣]+', '인상깊다')
  nouns <- str_replace_all(nouns, '민음사티비를+', 'mintv')
  nouns <- str_replace_all(nouns, '민음사[가-힣]+', '민음사')
  nouns <- str_replace_all(nouns, 'mintv', '민음사tv')
  nouns <- str_replace_all(nouns, '[가-힣]?인터뷰[가-힣]+', '인터뷰')
  nouns <- str_replace_all(nouns, '[가-힣]?고등학생[가-힣]+', '고등학생들')
  nouns <- str_replace_all(nouns, '책디자인이', 'bookd')
  nouns <- str_replace_all(nouns, '[가-힣]?디자인[가-힣]+', '디자인')
  nouns <- str_replace_all(nouns, 'bookd', '책디자인')
  nouns <- str_replace_all(nouns, '학교도서관[가-힣]+', '학교도서관')
  nouns <- str_replace_all(nouns, '느리', '늦다')
  nouns <- str_replace_all(nouns, '느[가-힣]+', '느낌')
  nouns <- str_replace_all(nouns, '로바꿔야할', '')
  nouns <- str_replace_all(nouns, '같아욥', '')
  nouns <- str_replace_all(nouns, '디자이너[가-힣]', '디자이너')
...
minl <- str_replace_all(minl, '인상[가-힣]+', '인상깊다')
str_extract_all(minl, '민음사[가-힣]+', simplify=T)
str_replace_all(minl, '민음사[가-힣]+', '민음사')
minl <- str_replace_all(minl, '민음사티비를+', 'mintv')
minl <- str_replace_all(minl, '민음사[가-힣]+', '민음사')
minl <- str_replace_all(minl, 'mintv', '민음사tv')
str_extract_all(minl, '민음사[가-힣]+', simplify=T)
str_extract_all(minl, '[가-힣]?인터뷰[가-힣]+', simplify=T)
minl <- str_replace_all(minl, '[가-힣]?인터뷰[가-힣]+', '인터뷰')
str_extract_all(minl, '[가-힣]?고등학생[가-힣]+', simplify=T)
minl <- str_replace_all(minl, '[가-힣]?고등학생[가-힣]+', '고등학생들')
str_extract_all(minl, '[가-힣]?디자인[가-힣]+', simplify=T)
minl <- str_replace_all(minl, '[가-힣]?고등학생[가-힣]+', '고등학생들')
minl <- str_replace_all(minl, '책디자인이', 'bookd')
minl <- str_replace_all(minl, '[가-힣]?디자인[가-힣]+', '디자인')
minl <- str_replace_all(minl, 'bookd', '책디자인')

str_extract_all(minl, '[가-힣]?도서관[가-힣]+', simplify=T)
minl <- str_replace_all(minl, '학교도서관[가-힣]+', '학교도서관')
